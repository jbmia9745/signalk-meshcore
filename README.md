...
- **Retries are configurable, and spacing them matters.** `dm_retries` (default 1) and `dm_retry_gap_seconds` (default 5) govern automatic re-sends of unconfirmed direct messages. Field loss-runs showed failures cluster in *time* (fade epochs lasting tens of seconds) rather than uniformly — a spaced retry samples different propagation, so a 60%-per-attempt hop becomes ~84% with one retry and climbs from there. Retries are new packets at the mesh layer (fresh ack codes), so repeater dedup does not suppress them.
