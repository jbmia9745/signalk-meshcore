...
- **Radio**: this plugin was developed and validated end-to-end on the **Heltec WiFi LoRa 32 V4** — the boat radio, crew radios, and the optional GNSS module were all V4 hardware, and every hardware note in this document (power budgets, GPS wiring, USB behavior) is V4-specific. Other MeshCore-supported boards should work over the same companion protocol, but they are **untested here**, and details like GPS support, BLE behavior, and TX current draw differ per board. If you don't want to debug hardware yourself, use the V4. (A repeater, if you add one, is plain mesh infrastructure the plugin never talks to directly — any MeshCore repeater hardware is fine; the validation boat used a RAK4631.) Flash **canonical MeshCore Companion firmware ≥ 1.15** from [flasher.meshcore.io](https://flasher.meshcore.io):
...
- **Recommended topology: below-decks radio + masthead repeater.** The boat radio lives at the server (below decks, USB power and data) where its RF reach is poor; a MeshCore repeater at the masthead bridges it to the world. Field-measured on the validation boat: every multi-hop echo of the boat's transmissions reached shore repeaters *via the masthead* — no shore station heard the below-decks radio directly. Repeater relaying is a firmware role, not per-contact behavior; the plugin needs no routing configuration, doesn't talk to the repeater, and any MeshCore repeater hardware works (RAK4631 on the validation boat). Repeaters can be administered remotely over the mesh (login + CLI from the phone app), so a masthead install doesn't need physical access afterward.
