...
- **Stale paths self-heal after a failure.** MeshCore stores one directed path per contact; when a repeater in that path moves or goes offline, every directed message to the contact fails and no layer of a headless node recovers on its own (the app-side self-heal is a phone/T-Deck behavior, absent on an unattended radio). So after a directed message exhausts its retries, the plugin resets that contact's stored path — the next send floods and relearns a working route (`PATH RESET <contact> after N failed attempts`), then optionally sends one recovery copy of the failed message. A per-contact cooldown (`reset_path_cooldown_minutes`, default 10) prevents reset storms to a genuinely offline node. Controlled by `reset_path_on_failure` (default on), `resend_after_path_reset` (default on), and `reset_recovery_gap_seconds` (default 3). This restores automatically what previously required a manual path reset. Channel pushes and telemetry polling are unaffected; the heal covers directed messages (command replies and crew alerts), which is where a pinned-but-dead route otherwise silently fails.
