# Design v2.1 — Auto Reset-Path on Directed DM Delivery Failure

**Status:** `[APPROVED FOR IMPLEMENTATION — johnrbek, 2026-06-20]`. Design is frozen. Implementation may proceed once the §3.1 Cerbo `resetPath` API check is run. Do not revise the design further unless that check contradicts the reset API assumption.
**Scope:** `plugin/device.js` (primary), `plugin/index.js` (config wiring + schema), `test/device.test.js`.
**Lineage:** v1 = `docs/design-reset-path-on-miss.md` (no recovery resend, no cooldown). v2 = `docs/design-reset-path-on-miss-v2.md` (added recovery/cooldown). This v2.1 supersedes v2 on approval and is the implementation reference.
...
## 0. Changes from v2 (text-only; architecture unchanged)

| # | Correction | Where |
|---|---|---|
| 1 | Config-structure language fixed: no flat `communications` schema group; new fields go in the existing top-level **`dms`** schema object; runtime reads normalized values from `settings.communications` via the spread-merge | §6.2 |
| 2 | All line-number references removed; replaced with symbol/block references | throughout |
| 3 | Queue-timeout claim reworded: bounded by the **configured** `CommandQueue` timeout (currently the **15000ms default**, because `index.js` constructs `CommandQueue` with `undefined` timeoutMs); queue timeout does **not** cancel the underlying meshcore.js promise/listener | §3, §3.1, §8 |
| 4 | Detached recovery resend MUST `.catch` send failures (no unhandled rejection) | §6.1, §9 |
| 5 | Cooldown keyed by **full public-key hex** internally; 6-byte prefix used **only** for logs | §6.1 |
| 6 | `resetAttempts` is intentionally **in-memory, process-lifetime only** for v1; rationale stated | §6.1, §8 |
| 7 | `nowMs` injection is **new test plumbing** — current `device.js` does not inject it | §6.1, §9 |
| 8 | Test-baseline claim removed; design specifies **required cases**, not a green count, until the full repo test count is confirmed | §9 |
| 9 | `resetPath` API MUST be verified on the target runtime before coding; wrapper fallback defined | §3.1, §12 |
